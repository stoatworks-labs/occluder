#!/usr/bin/env bash
# release-local.sh — cut a full Occluder release from this Mac.
#
...
# The CMake value is what the DAW reports and what the About panel shows, and
# it drifts from the tags unless something writes it back - so the tag is the
# source of truth and this writes it into CMakeLists.
current="$(git describe --tags --abbrev=0 2>/dev/null | sed 's/^v//')"
[[ -z "$current" ]] && current="$(sed -n 's/^project(Occluder VERSION \([0-9.]*\).*/\1/p' CMakeLists.txt)"
[[ -z "$version" ]] && version="$(awk -F. '{printf "%d.%d.%d", $1, $2, $3+1}' <<<"$current")"
cmake_ver="$(sed -n 's/^project(Occluder VERSION \([0-9.]*\).*/\1/p' CMakeLists.txt)"
tag="v${version}"
echo "==> Occluder ${current} -> ${version} (CMakeLists said ${cmake_ver})"
sed -i '' "s/^project(Occluder VERSION ${cmake_ver}/project(Occluder VERSION ${version}/" CMakeLists.txt

rl_init "Occluder" occluder "$version" com.stoatworks.occluder "$out"
...
art="build-release/Occluder_artefacts/Release"
vst3="$art/VST3/Occluder.vst3"
au="$art/AU/Occluder.component"
app="$art/Standalone/Occluder.app"

# Trust lipo, not the build log — a single-tone test cannot tell you the
# architecture and neither can a successful compile.
for b in "$vst3/Contents/MacOS/Occluder" "$au/Contents/MacOS/Occluder" "$app/Contents/MacOS/Occluder"; do
  [[ -f "$b" ]] || continue
  a="$(lipo -archs "$b")"
...
if (( use_vm )) && command -v prlctl >/dev/null 2>&1 \
  ...
  bash "$repo/scripts/release-windows-vm-cmake.sh" "$repo" occluder "'*.vst3','*.exe'" \
...
cat <<'NOTE'
    ...
    Unsigned. macOS users must run
      xattr -dr com.apple.quarantine "/Library/Audio/Plug-Ins/VST3/Occluder.vst3"
    ...
    installs Occluder into this Mac's own plugin folders.
NOTE
...
if (( upload )); then
  ...
  git tag -a "$tag" -m "Occluder ${version}" || true
  git push origin HEAD --tags
  gh release create "$tag" --title "Occluder ${version}" \
